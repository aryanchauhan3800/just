...
import PurchasePaymentTabs from './PurchasePaymentTabs'
import PurchasePaymentMadeModal from './PurchasePaymentMadeModal'

const PurchasePaymentModal = ({ open, onOpenChange }: InvoiceDrawerProps) => {
    ...
    return (
        <>
                    ...
                    <DialogHeader className="flex-shrink-0">
                        <div className='flex flex-row justify-between items-center border-b py-4 px-6'>
                            <DialogTitle className='text-lg text-[#474747] flex items-center gap-2'>
                                <RiFileList3Line />Payment to Vendor
                            </DialogTitle>
                            <DialogClose asChild>
                            ...
                            <div className="w-full overflow-hidden">
                                ...
                                >
                                    <div className="snap-start min-w-full flex-shrink-0">
                                        <p className='text-[#6B6B6B] text-lg mb-2'>Purchase Amount Payment</p>
                                        <div className='border rounded-xl flex flex-row w-full'>
                                            <div className='px-4 py-3 w-1/2'>
                                                <p className='text-[#242424] text-sm'>To Pay <span className='text-[#8F8F8F] text-xs'>for this Purchase Order</span></p>
                                                <p className='text-[#474747] text-[22px]'>10,000.00</p>
                                            </div>
                                            <div className='px-4 py-3 w-1/2 border-l'>
                                                <p className='text-[#8F8F8F] text-xs flex gap-1'><CiBank className='h-[14px] w-[14px]' />Paying from</p>
                                            </div>
                                        </div>
                                    </div>
            ...
            <PurchasePaymentMadeModal open={showMadePaymentModal} onOpenChange={setShowMadePaymentModal} />
        </>
    )
}
