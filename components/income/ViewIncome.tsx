...
type IncomeItem = { name: string; date: string };

const MockIncomeArray: IncomeItem[] = [
	{ name: "IT & Services Charge", date: "12 Mar, 2025" },
	{ name: "Meals & Entertainment", date: "26 Mar, 2025" },
	{ name: "Travel Cost", date: "8 Feb, 2025" },
	{ name: "Labour Income", date: "12 Jan, 2025" },
	{ name: "Repair & Maintenance", date: "24 Apr, 2025" },
];

const MockCredsArray = [
	{ title: "Income Date", value: "12 Apr, 2025" },
	{ title: "Payment Mode", value: "Cash" },
];

const MockIncomeCategoryArray = [
	{ name: "IT & Services Charge", amount: "10,000" },
	{ name: "Labour Charges", amount: "10,000" },
	{ name: "Repair & Incomes", amount: "30,000" },
	{ name: "Cleaning & Janitorial Income", amount: "10,000" },
];

const ViewIncome: FC<ViewIncomeProps> = ({}) => {
	const router = useRouter();
	const [showRemoveIncomeDialog, setShowRemoveIncomeDialog] =
		...
		<div className="flex flex-col justify-between h-full overflow-hidden">
			...
			<div className="flex justify-between items-center min-h-[64px] px-5 bg-white border-b border-[#E8E8E8]">
				<div className="flex gap-x-5 items-center">
					...
					<h2 className="text-xl text-[#474747]">Income</h2>
				</div>
					...
					<DropdownMenu>
						...
						<DropdownMenuContent className="w-[300px] -ml-60">
							...
							<DropdownMenuItem
								onClick={() =>
									setTimeout(() => {
										setShowRemoveIncomeDialog(true);
									}, 200)
								}
							>
							...
							<div className="flex flex-col bg-white max-h-[162px] flex-1 border border-[#E8E8E8] rounded-[8px]">
								...
								<div className="p-6 flex justify-between items-start flex-1">
									<div className="flex gap-4 items-center">
										{/* circle */}
										<span className="bg-success text-white border-4 rounded-full center shadow-sm size-12">
											S
										</span>
									...
									<div className="!w-[220px] bg-[#FAFAFA] !h-[68px] border-l-4 border-l-success px-4 flex flex-col justify-center">
										<span className="text-sm text-[#8F8F8F]">
											Income Amount
										</span>
										<span className="text-[#242424] text-[22px]">
			...
			{/* delete Income popup */}
			<ConfirmationDialog
				open={showRemoveIncomeDialog}
				onOpenChange={setShowRemoveIncomeDialog}
				acceptBtnFx={async () => {
					setShowRemoveIncomeDialog(false);
					return false;
				}}
				description="Are you sure you want to delete the Income?"
				acceptText={"Yes, Delete Income"}
...
type IncomeSidebarItemProps = {
	isActive?: boolean;
	setActive: () => void;
	data: IncomeItem;
};

const IncomeSidebarItem: FC<IncomeSidebarItemProps> = ({
	isActive,
	setActive,
	data,
}) => {
	return (
		...
		>
			...
			<div className="flex-1 px-6 gap-4 py-4 flex items-center">
				<span className="!size-8 bg-success text-white center rounded-full uppercase">
					{data.name.at(0)}
				</span>
