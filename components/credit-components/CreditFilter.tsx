...
interface CreditTableWithFilter {
    open: boolean;
    ...
    filters: {
        ...
        withToPay: boolean
        withToReceive: boolean
    };
    setFilters: React.Dispatch<React.SetStateAction<any>>;
}


const CreditFilter = ({ open, onOpenChange, filters, setFilters }: CreditTableWithFilter) => {
                ...
                <div className="h-full flex flex-col">
                    ...
                    <div className="flex-1 overflow-y-auto space-y-4">
                            ...
                            <div className="flex flex-wrap px-6">
                                <FormControlLabel
                                    control={
                                        <Checkbox
                                            size="small"
                                            checked={filters.statuses.includes("Fully Adjusted")}
                                            onChange={(e) => {
                                                const checked = e.target.checked;
                                                setFilters((prev) => ({
                                                    ...prev,
                                                    statuses: checked
                                                        ? [...prev.statuses, "Fully Adjusted"]
                                                        : prev.statuses.filter((s) => s !== "Fully Adjusted"),
                                                }));
                                            }}
                                        />
                                    }
                                    label={<Badge variant="outline"
                                        className='px-3 py-1 bg-green-100 rounded-full text-sm text-[#22B947]'>Fully Adjusted</Badge>}
                                />
                                <FormControlLabel
                                    control={
                                        <Checkbox
                                            size="small"
                                            checked={filters.statuses.includes("Partially Adjusted")}
                                            onChange={(e) => {
                                                const checked = e.target.checked;
                                                setFilters((prev) => ({
                                                    ...prev,
                                                    statuses: checked
                                                        ? [...prev.statuses, "Partially Adjusted"]
                                                        : prev.statuses.filter((s) => s !== "Partially Adjusted"),
                                                }));
                                            }}
                                        />
                                    }
                                    label={<Badge variant="outline"
                                        className='px-3 py-1 rounded-full text-sm bg-yellow-100 text-[#FAAD14]'>Partially Adjusted</Badge>}
                                />
                                <FormControlLabel
                                    control={
                                        <Checkbox
                                            size="small"
                                            checked={filters.statuses.includes("Not Adjusted")}
                                            onChange={(e) => {
                                                const checked = e.target.checked;
                                                setFilters((prev) => ({
                                                    ...prev,
                                                    statuses: checked
                                                        ? [...prev.statuses, "Not Adjusted"]
                                                        : prev.statuses.filter((s) => s !== "Not Adjusted"),
                                                }));
                                            }}
                                        />
                                    }
                                    label={<Badge variant="outline"
                                        className='px-3 py-1 rounded-full text-sm bg-red-100 text-[#F5222D]'>Not Adjusted</Badge>}
                                />
                            </div>
                            <div className='border-t pt-2'>
                                <FormControlLabel
                                    control={
                                        <Checkbox
                                            size="small"
                                            checked={filters.withToPay}
                                            onChange={(e) => {
                                                const checked = e.target.checked;
                                                setFilters((prev) => ({ ...prev, withToPay: checked }));
                                            }}
                                        />
                                    }
                                    label={<span className="text-sm">Show notes TO Pay</span>}
                                    className="px-6"
                                />
                                <hr />
                                <FormControlLabel
                                    control={
                                        <Checkbox
                                            size="small"
                                            checked={filters.withToReceive}
                                            onChange={(e) => {
                                                const checked = e.target.checked;
                                                setFilters((prev) => ({ ...prev, withToReceive: checked }));
                                            }}
                                        />
                                    }
                                    label={<span className="text-sm">Show noted To Recieve</span>}
                                    className="px-6 pt-2"
                                />
                            </div>
                        </Card>
                        ...
                        <Card className='p-2 rounded-none border-none shadow-none'>
                            ...
                            >
                                <Typography className="text-base font-medium">₹ Adjustment Amount wise</Typography>
                        ...
                        <Card className='p-2 border-0 border-b rounded-none shadow-none'>
                            ...
                            >
                                <Typography className="text-base font-medium">Show Orders Created</Typography>
